...
export function getRecentMessages(): { id: number; author: string; content: string; created_at: string }[] {
  return getRecentMessagesStmt.all() as { id: number; author: string; content: string; created_at: string }[];
}

export function insertMessage(author: string, content: string): number {
  return insertMessageStmt.run(author, content).lastInsertRowid as number;
}
...
// ── Schema migration: add panel_id to runtime_device_configs ──

{
  const rows = db.prepare("PRAGMA table_info(runtime_device_configs)").all() as { name: string }[];
  if (!rows.some((r) => r.name === "panel_id")) {
    db.run("ALTER TABLE runtime_device_configs ADD COLUMN panel_id TEXT DEFAULT ''");
  }
}
...
const getRuntimeDeviceConfigsStmt = db.prepare(`
  SELECT token, device_id, device_name, platform, panel_id
...
const upsertRuntimeDeviceConfigStmt = db.prepare(`
  INSERT INTO runtime_device_configs (token, device_id, device_name, platform, panel_id, updated_at)
  VALUES (?, ?, ?, ?, ?, datetime('now'))
  ON CONFLICT(device_id) DO UPDATE SET
