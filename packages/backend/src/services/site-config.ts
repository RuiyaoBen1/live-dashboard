...
import { getPanels, type PanelDef } from "../middleware/auth";
...
function toMusicTrack(value: unknown): MusicTrack | undefined {
  if (!value || typeof value !== "object") return undefined;
  const record = value as Record<string, unknown>;
  const title = nonEmpty(typeof record.title === "string" ? record.title : undefined);
  const url = nonEmpty(typeof record.url === "string" ? record.url : undefined);
  if (!title || !url) return undefined;
  const artist = nonEmpty(typeof record.artist === "string" ? record.artist : undefined);
  const cover = nonEmpty(typeof record.cover === "string" ? record.cover : undefined);
  return { title, url, ...(artist ? { artist } : {}), ...(cover ? { cover } : {}) };
}

function getMusicPlaylist(): MusicTrack[] {
  const raw = nonEmpty(process.env.MUSIC_PLAYLIST);
  if (!raw) return [];
  const parsed = parseDashboardList(raw);
  return parsed
    .map((entry) => toMusicTrack(entry))
