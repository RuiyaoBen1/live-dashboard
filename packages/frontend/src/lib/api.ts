...
export async function fetchConfig(
  ...
  try {
    ...
    const panels = Array.isArray(data.panels)
      ? (data.panels as unknown[])
          .filter((p: unknown): p is PanelDef => {
            if (!p || typeof p !== "object") return false;
            const r = p as Record<string, unknown>;
            return typeof r.id === "string" && typeof r.name === "string";
          })
      : defaultConfig.panels;
    const musicPlaylist = Array.isArray(data.musicPlaylist)
      ? (data.musicPlaylist as unknown[])
          .filter((t: unknown): t is MusicTrack => {
            if (!t || typeof t !== "object") return false;
            const r = t as Record<string, unknown>;
            return typeof r.title === "string" && typeof r.url === "string";
          })
      : defaultConfig.musicPlaylist;
    return {
...
export async function fetchStats(
  ...
  const params = new URLSearchParams({ panel_id: panelId, days: String(days) });
  const url = withQuery("/api/stats", params);
  return fetchJsonWithTimeout<StatsResponse>(url, signal);
}
...
export async function fetchPanelSummary(
  ...
  const params = new URLSearchParams({ panel_id: panelId });
  const url = withQuery("/api/panel-summary", params);
  return fetchJsonWithTimeout<PanelSummaryResponse>(url, signal);
}
...
export interface VisitorWeatherResponse {
  weather: WeatherData | null;
}

export async function fetchVisitorWeather(): Promise<VisitorWeatherResponse> {
  try {
    return await fetchJsonWithTimeout<VisitorWeatherResponse>("/api/visitor-weather", undefined, 5000);
  } catch {
    return { weather: null };
  }
}
...
export async function fetchWeather(city: string): Promise<WeatherData> {
  ...
  return fetchJsonWithTimeout<WeatherData>(`/api/weather?${params}`);
}

export async function fetchWeatherLocations(): Promise<WeatherLocation[]> {
  const res = await fetchJsonWithTimeout<{ locations: WeatherLocation[] }>("/api/weather-locations");
...
export async function fetchMessages(): Promise<Message[]> {
  const res = await fetchJsonWithTimeout<{ messages: Message[] }>("/api/messages");
  return res.messages;
}
...
export async function deleteMessage(id: number | "all", password: string): Promise<void> {
  const res = await fetch("/api/messages", {
    method: "DELETE",
    headers: { "Content-Type": "application/json" },
