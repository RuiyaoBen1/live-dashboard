...
import { fetchWeather, fetchWeatherLocations, fetchVisitorWeather, type WeatherData, type WeatherLocation } from "@/lib/api";
...
function HomeInner({
  ...
  const [visitorWeather, setVisitorWeather] = useState<WeatherData | null>(null);
  const [visitorWeatherDesc, setVisitorWeatherDesc] = useState<string | undefined>();
  ...
  useEffect(() => {
    fetchWeatherLocations().then(setWeatherLocations).catch(() => {});
  }, []);

  useEffect(() => {
    const load = () => {
      fetchVisitorWeather().then((res) => {
        if (res.weather) {
          setVisitorWeather(res.weather);
          setVisitorWeatherDesc(res.weather.weather_desc);
        }
      }).catch(() => {});
    };
    load();
  ...
  useEffect(() => {
    ...
    const city = weatherLocations[0].city;
    const load = () => {
      fetchWeather(city)
        .then((data) => setWeatherDesc(data.weather_desc))
        .catch(() => {});
    };
    load();
    const interval = setInterval(load, 10 * 60 * 1000);
  ...
  const [mobilePage, setMobilePage] = useState<"dashboard" | "weather" | "messages">("dashboard");
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetchPanelSummary(panelId).then((data) => {
      if (!cancelled) setPanelSummary(data.summary);
    }).catch(() => {});
  ...
  const devices = useMemo(() => {
    ...
    const filtered = selectedPanelId
      ? list.filter((d) => (d.panel_id || "") === selectedPanelId)
      : list;
    return [...filtered].sort((left, right) => left.device_id.localeCompare(right.device_id));
  }, [current?.devices, selectedPanelId]);
  ...
  return (
    <>
      <Header
        ...
        weatherDesc={visitorWeatherDesc ?? weatherDesc}
        allOffline={allOffline}
        panelSummary={panelSummary}
      />

    {/* Desktop: 3-column layout */}
    <div className="hidden lg:grid grid-cols-[220px_1fr_260px] gap-4">
      {/* Left sidebar: Weather */}
      <aside className="space-y-2 sticky top-4 self-start">
        <WeatherCard isVisitor visitorWeather={visitorWeather} />
        {weatherLocations.length > 0 && weatherLocations.map((loc) => (
          <WeatherCard key={loc.city} location={loc} />
        ))}
      </aside>
      ...
      <DashboardSwitcher
        ...
        onSelect={(id) => { setSelectedDashboardId(id); setSelectedPanelId(null); }}
        panels={panels}
      ...
      <section className="mb-6 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {panels.length > 0 ? (
          panels.map((panel) => {
            const panelDevices = (current?.devices ?? []).filter((d) => (d.panel_id || "") === panel.id);
            const onlineCount = panelDevices.filter((d) => d.is_online === 1).length;
            const activeDevice = panelDevices.find((d) => d.is_online === 1) ?? panelDevices[0];
            const panelOwner = panel.name.replace(/的面板$/, "");
            const activeLabel = activeDevice
              ? activeDevice.is_online === 1
                ? activeDevice.app_name === "idle" ? `${panelOwner}正在摸鱼` : `${panelOwner}正在${activeDevice.app_name || ""}`
                : `${panelOwner}离线中`
              : "暂无设备";
            const statusText = onlineCount > 0 ? "在线" : panelDevices.length > 0 ? "离线" : "暂无设备";
            return (
              <PanelOverviewCard
                key={panel.id}
                panel={panel}
                onlineDevices={onlineCount}
                totalDevices={panelDevices.length}
      ...
      {current && (
        <>
          <CurrentStatus
            ...
            displayName={selectedPanelId ? panels.find((p) => p.id === selectedPanelId)?.name.replace(/的面板$/, "") : activeDashboard?.name}
          />
      ...
      <footer className="mt-12 pt-4 separator-dashed text-center">
        <p className="text-[10px] text-[var(--color-text-muted)]">
          Status Now &middot; 已接入 {resolvedSnapshots.length} 个面板 &middot; 状态 10 秒刷新 / 时间线 30 秒刷新 &middot; (◕ᴗ◕)
        </p>
      </footer>
      </div>

      {/* Right sidebar: Message Board */}
      <aside className="sticky top-4 self-start">
        <MessageBoard />
      </aside>
    </div>

    {/* Mobile: single page with bottom nav */}
    <div className="lg:hidden pb-16">
      {mobilePage === "dashboard" && (
        <div className="min-w-0">
          ...
          <section className="mb-6 grid gap-3 grid-cols-2">
            {panels.length > 0 ? (
              panels.map((panel) => {
                const panelDevices = (current?.devices ?? []).filter((d) => (d.panel_id || "") === panel.id);
                const onlineCount = panelDevices.filter((d) => d.is_online === 1).length;
      ...
      {mobilePage === "weather" && (
        <div className="space-y-3">
          <h2 className="text-sm font-bold text-[var(--color-text-muted)] uppercase tracking-wider">天气</h2>
          <WeatherCard isVisitor visitorWeather={visitorWeather} />
          {weatherLocations.length > 0 ? weatherLocations.map((loc) => (
            <WeatherCard key={loc.city} location={loc} />
          )) : (
            <p className="text-xs text-[var(--color-text-muted)] text-center py-8">暂无天气配置</p>
          )}
        </div>
      )}
      ...
      {mobilePage === "messages" && (
        <div>
          <h2 className="text-sm font-bold text-[var(--color-text-muted)] uppercase tracking-wider mb-3">留言板</h2>
          <MessageBoard />
        </div>
      )}
    </div>
    ...
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 backdrop-blur-md bg-[var(--color-card)]/80 border-t border-[var(--color-border)]">
      <div className="flex justify-around items-center h-12">
        <button
          type="button"
          onClick={() => setMobilePage("dashboard")}
        ...
        <button
          type="button"
          onClick={() => setMobilePage("messages")}
          className={`flex-1 h-full text-xs font-medium transition-colors ${
            mobilePage === "messages"
              ? "text-[var(--color-primary)]"
              : "text-[var(--color-text-muted)]"
...
function DashboardSwitcher({
  ...
  const externalDashboards = hasPanels
    ? dashboards.filter((d) => !d.isPrimary)
    : dashboards;
      ...
      <div className="flex flex-wrap gap-2">
        {hasPanels && (
          <button
            ...
            className={`panel-chip ${selectedPanelId === null ? "panel-chip-active" : ""}`}
          >
            <span>全部</span>
            <span className="text-[10px] opacity-70">{devices.length}</span>
          </button>
        )}
        {panels.map((panel) => {
          const count = devices.filter((d) => (d.panel_id || "") === panel.id).length;
          return (
            <button
              key={panel.id}
              type="button"
              onClick={() => onPanelSelect(panel.id)}
              className={`panel-chip ${selectedPanelId === panel.id ? "panel-chip-active" : ""}`}
            >
        ...
        {externalDashboards.map((dashboard) => (
          <button
            key={dashboard.id}
            type="button"
            onClick={() => onSelect(dashboard.id)}
            className={`panel-chip ${dashboard.id === selectedDashboardId && selectedPanelId === null ? "panel-chip-active" : ""}`}
          >
...
function PanelOverviewCard({
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-[var(--color-text)]">{panel.name}</p>
        </div>
        <span className={`status-pill ${onlineDevices > 0 ? "status-pill-online" : "status-pill-offline"}`}>
          {statusText}
        </span>
      </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 text-center">
        <div>
          <p className="text-[10px] uppercase tracking-wide text-[var(--color-text-muted)]">Devices</p>
          <p className="text-lg font-semibold text-[var(--color-text)]">{onlineDevices}/{totalDevices}</p>
        </div>
        <div>
          <p className="text-[10px] uppercase tracking-wide text-[var(--color-text-muted)]">Viewers</p>
          <p className="text-lg font-semibold text-[var(--color-text)]">{viewerCount}</p>
        </div>
        <div>
          <p className="text-[10px] uppercase tracking-wide text-[var(--color-text-muted)]">Status</p>
          <p className="text-sm font-semibold text-[var(--color-text)] truncate">{activeLabel}</p>
        </div>
      </div>
    </button>
  );
